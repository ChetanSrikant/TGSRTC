...
export default function Page10m() {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const toggleSidebar = () => setSidebarOpen(prev => !prev);

  const [keys, setKeys] = useState([]);
  const [formData, setFormData] = useState({ df_key: 'ALL', forecast_days: 7 });
  ...
  useEffect(() => {
    const fetchKeys = async () => {
      try {
        const response = await fetch('/api/keys10');
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/forecast10', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...formData, start_date: "2023-12-01", end_date: "2024-12-31" })
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || 'Forecast API error');
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <main className="flex-grow p-6 flex flex-col bg-gray-50 overflow-y-auto">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border mb-6 flex-shrink-0">
          <h1 className="text-xl font-bold mb-4 text-gray-800">Forecast Parameters for OPRS</h1>
